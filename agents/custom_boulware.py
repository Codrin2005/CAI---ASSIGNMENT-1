...
class Group34_Negotiator_Boulware(SAONegotiator):
    ...
    def __call__(self, state: SAOState) -> SAOResponse:
        ...
        candidates = [o for o in self._outcomes if float(self.ufun(o)) <= target]
        if not candidates:
            chosen = self._outcomes[-1]
        else:
            chosen = self._rng.choice(candidates[: min(5, len(candidates))])
